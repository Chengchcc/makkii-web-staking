...
function hexToBn(hex) {
  ...
  if (0x80 & highbyte) {
    ...
    bn = JSBI.add(JSBI.BigInt('0b' + bn.toString(2).split('').map(function (i) {
      return '0' === i ? 1 : 0
    }).join('')), JSBI.BigInt(1));
    // add the sign character to output string (bytes are unaffected)
    bn = JSBI.unaryMinus(bn);
  }
...
function bitnot(bn) {
  ...
  return JSBI.add(JSBI.BigInt('0b' + prefix + bin), JSBI.BigInt(1));
}
...
function bufToBn(buf) {
  ...
  return JSBI.BigInt('0x' + hex.join('')).toString(10);
}
