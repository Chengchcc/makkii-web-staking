...
prompt.get(schema, (err, result) => {
    ...
    console.log(`${colors.green('=============== try to pack dist ====================')}`)
    exec('tar -czvf dist.tar.gz dist', (err1) => {
        if (err1) {
            console.log("Something's wrong")
            console.log(err1)
            return;
        }
        console.log('   pack dist ok')
        ...
        ssh.connect(config).then(() => {
            ...
            const remotePath = `${result.remotePath}/dist.tar.gz`;
            console.log(`${colors.green('=============== try to upload dist.tar.gz ================')}`)
            console.log(`  localPath: ${localPath}`);
            console.log(`  remotePath: ${remotePath}`);
            ssh.putFile(localPath, remotePath).then(() => {
                console.log("   upload ok")
                console.log(`${colors.green('=============== try to unpack dist.tar.gz ==============')}`)
                ssh.execCommand(`rm -rf ${targetDir}; tar -xzvf dist.tar.gz; mv dist ${targetDir}; rm dist.tar.gz`, { cwd: result.remotePath }).then(r => {
                    if (r.stderr) {
                        console.log("   Something's wrong")
